...
namespace recis {
namespace optim {
struct SparseAdamWTFOptions
    : public SparseOptimizerCloneableOptions<SparseAdamWTFOptions> {
  SparseAdamWTFOptions(double lr = 1e-3);
  TORCH_ARG(double, lr) = 1e-3;
  typedef std::tuple<double, double> betas_t;
  TORCH_ARG(betas_t, betas) = std::make_tuple(0.9, 0.999);
  TORCH_ARG(double, eps) = 1e-8;
  TORCH_ARG(double, weight_decay) = 1e-2;
  TORCH_ARG(bool, use_nesterov) = false;

 public:
  double get_lr() const override;
  void set_lr(const double lr) override;
};
struct TORCH_API SparseAdamWTFParamState
    : public SparseOptimizerCloneableParamState<SparseAdamWTFParamState> {
 public:
  SparseAdamWTFParamState()
      : step_dtype_(torch::kInt64), beta_dtype_(torch::kFloat) {}
  ...
  TORCH_API friend bool operator==(const SparseAdamWTFParamState &lhs,
                                   const SparseAdamWTFParamState &rhs);
 ...
 private:
  ...
  ParamContainer exp_avg_;
  ParamContainer exp_avg_sq_;
  torch::Dtype step_dtype_;
  torch::Dtype beta_dtype_;
  HashTablePtr hashtable_;
};

class SparseAdamWTF : public SparseOptimizer {
 public:
  explicit SparseAdamWTF(std::vector<SparseOptimizerParamGroup> param_groups,
                         SparseAdamWTFOptions defaults = {})
      : SparseOptimizer(std::move(param_groups),
                        std::make_unique<SparseAdamWTFOptions>(defaults)) {
  ...
  explicit SparseAdamWTF(std::unordered_map<std::string, HashTablePtr> params,
                         SparseAdamWTFOptions defaults = {})
      : SparseAdamWTF({SparseOptimizerParamGroup(std::move(params))},
                      defaults) {}
